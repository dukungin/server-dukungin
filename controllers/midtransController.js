...
// ============================================================
// REQUEST WITHDRAWAL — MANUAL (Tanpa integrasi pihak ketiga)
...
exports.requestWithdrawal = async (req, res) => {
  ...
  const MIN_BALANCE = 20000; // saldo minimum untuk bisa tarik
  const referenceNo = `wd-${userId}-${Date.now()}`;
  ...
  try {
    // Cek saldo minimum 20k DAN cukup untuk amount + fee, potong atomic
    const user = await User.findOneAndUpdate(
      {
        _id: userId,
    ...
    console.log(`[requestWithdrawal] @${user.username} ajukan WD Rp${amt} via ${channelCode}`);

    res.json({
      message: 'Permintaan penarikan berhasil diajukan. Dana akan diproses admin dalam 1×24 jam.',
      referenceNo,
      status: 'PENDING',
    });
...
// Menampilkan semua riwayat withdrawal milik streamer
// ============================================================
exports.getWithdrawalHistory = async (req, res) => {
  const userId = req.user.id;
...
// ============================================================
// [ADMIN] GET SEMUA WITHDRAWAL (semua status)
// ============================================================
exports.adminGetPendingWithdrawals = async (req, res) => {
...
// COMPLETED = admin sudah transfer manual → notif streamer
// FAILED    = ditolak → saldo dikembalikan + notif streamer
// ============================================================
exports.adminUpdateWithdrawal = async (req, res) => {
  const { id } = req.params;
  const { status, note } = req.body; // note opsional, alasan reject dll
  ...
  try {
    ...
    if (status === 'COMPLETED') {
      // Admin sudah transfer manual → hanya update status, notif ke streamer
      ...
      // Kembalikan saldo ke streamer (amount + fee)
      const refundAmount = parseFloat(withdrawal.amount) + 5000;
